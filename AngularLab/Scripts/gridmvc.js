...
GridMvc = (function ($) {
    ...
    gridMvc.prototype.init = function () {
        ...
        this.addFilterWidget(new EnumFilterWidget());
        this.openedMenuBtn = null;
        this.resetEvent();

    };

    gridMvc.prototype.resetEvent = function () {
        this.initFilters();
        this.initCustomColumns();
        //init fixedHeader
        this.jqContainer.find(".grid-table[data-fixed-header=True]").fixedHeader();
    ...
    */
    ...
    gridMvc.prototype.onCustomSaved = function (func) {
        this.events.push({ name: "onCustomSaved", callback: func });
    };

    gridMvc.prototype.notifyCustomSaved = function () {
        this.notifyEvent("onCustomSaved", null);
    };
    ...
    gridMvc.prototype.openFilterPopup = function (self, html) {
        ...
        if (typeof (widget.onRender) != 'undefined')
            widget.onRender(widgetContainer, self.lang, columnType, filterDataObj, function (values) {
                ...
                if (self.isAjax)
                    self.applyAjaxFilterValues(filterUrl, columnName, values, false);
                else
        ...
        if ($(this).find(".grid-filter-btn").hasClass("filtered") && widget.showClearFilterButton()) {
            ...
            inner.find(".grid-filter-clear").click(function () {
                if (self.isAjax)
                    self.applyAjaxFilterValues(filterUrl, columnName, "", true);
                else
    ...
    */
    ...
    gridMvc.prototype.addCustomFilter = function (widgetName, nameField, valueField, url) {
        /// <signature>
        /// <summary>addCustomFilter</summary>
        /// <param name="widgetName" type="String">識別名稱須與Columns.SetWidget一樣</param>
        /// <returns type="CustomFilterWidget" />
        /// </signature>
        this.filterWidgets.push(new CustomFilterWidget({ widgetName: widgetName, name: nameField, value: valueField, url: url }));
    };
    /***
    ...
    */
    ...
    gridMvc.prototype.initCustomColumns = function () {
        var self = this;
        this.jqContainer.find(".grid-custom-column").each(function () {
            var customAcion = $(this).attr("data-custom-action");
            $(this).click(function () {
                self.openCusomColumnsPopup.call(this, self, customAcion);
            });
        });
    };
    gridMvc.prototype.openCusomColumnsPopup = function (self, customAcion) {
        sysCore.bootstrap.dialog({
            remoteUrl: customAcion,
            remoteCallback: function () {
                $("#gridColAdd").on("click", colAdd);
                $("#gridColRemove").on("click", colRemove);
                $("#gridColUp").on("click", colUp);
                $("#gridColDown").on("click", colDown);
                $("#gridChooseTab a:last").on("click", nextOrder);
            },

            size: "lg",//lg|sm
            buttons: {
                success: { label: "Cancel", className: "btn-default", }
                , main: {
                    label: "Save & Close", className: "btn-primary",
                    callback: function (e) {
                        $("#gridSelectCol option").prop("selected", true);
                        //重排排序表單的name,先加100避開name重覆
                        $("#gridSelectSortTable tr:gt(0)").each(function (i, tr) {
                            $(tr).find(":hidden").attr("name", "selectOrderbys[" + i + 100 + "].ColumnName");
                            $(tr).find(":radio").attr("name", "selectOrderbys[" + i + 100 + "].SortDirection");
                        })
                        $("#gridSelectSortTable tr:gt(0)").each(function (i, tr) {
                            $(tr).find(":hidden").attr("name", "selectOrderbys[" + i + "].ColumnName");
                            $(tr).find(":radio").attr("name", "selectOrderbys[" + i + "].SortDirection");
                        })

                        sysCore.ajaxHelper.postForm({
                            formselector: $("#gridChooseWrap").closest("form")
                            , postUrl: customAcion
                          , callback: function (response) {
                              self.gridSort = null;//清空排序
                              self.notifyCustomSaved();
                ...
                , orderby: { label: "Next setting sort", className: "btn-info", callback: nextOrder }
    ...
    function nextOrder() {
        $("#gridChooseTab a:last").tab("show");
        var selectCols = $("#gridSelectCol option");
        var $divUnSelectOrderby = $("#divUnSelectOrderby");
        var template = $("#orderbyTpl").html();
        $divUnSelectOrderby.empty();
        //加入未存在已排序的清單
        $.each(selectCols, function (i, e) {
            var colName = $(e).val();
            var selectOrder = $("#gridSelectSortTable a[grid-attr=remove][grid-col=" + colName + "]");
            var $copyTpl = $(template);
            $copyTpl.attr("grid-col", colName)
            $copyTpl.find("[grid-attr=title]").html($(e).text());
            $copyTpl.find("[grid-attr=add]").attr("grid-col", colName).on("click", sortAdd);
            $divUnSelectOrderby.append($copyTpl);
        ...
        //移除已排序但未在顯示清單
        $("#gridSelectSortTable tr[grid-col]").each(function (j, se) {
            var orderColName = $(se).attr("grid-col");
            var obj = $("#gridSelectCol option[value=" + orderColName + "]");
            if (obj.length == 0)
                se.remove();
        })
        registerRemove();
    ...
    function sortAdd() {
        var colName = $(this).attr("grid-col");
        var $template = $($("#addOrderbyTpl").html()).find("tr[grid-col=" + colName + "]");
        $("#divUnSelectOrderby div[grid-col=" + colName + "]").fadeOut(100);
        $("#gridSelectSortTable").append($template);
        registerRemove();
        registerOrderSort();
    }

    function registerRemove() {
        $("#gridSelectSortTable a[grid-attr=remove]").unbind("click");
        $("#gridSelectSortTable a[grid-attr=remove]").each(function (i, e) {
            $(e).on("click", function () {
                var colName = $(this).attr("grid-col");
                $("#divUnSelectOrderby div[grid-col=" + colName + "]").fadeIn(100);
                $("#gridSelectSortTable tr[grid-col=" + colName + "]").fadeOut(100, function () { $(this).remove(); });
            });
        })

    }

    function registerOrderSort() {
        $("#gridSelectSortTable button[grid-attr=upOrderby]").unbind("click");
        $("#gridSelectSortTable button[grid-attr=downOrderby]").unbind("click");
        $("#gridSelectSortTable button[grid-attr=upOrderby]").on("click", function () {
            var colName = $(this).attr("grid-col");
            var current = $("#gridSelectSortTable tr[grid-col=" + colName + "]");
            var index = $("#gridSelectSortTable tr").index(current);
            if (index > 1) {
                var clone = current.clone(true, true);
                current.remove();
                index = index - 1;
                clone.insertBefore($("#gridSelectSortTable tr:eq(" + index + ")"));
            }
        })

        $("#gridSelectSortTable button[grid-attr=downOrderby]").on("click", function () {
            var colName = $(this).attr("grid-col");
            var current = $("#gridSelectSortTable tr[grid-col=" + colName + "]");
            var index = $("#gridSelectSortTable tr").index(current);
            if (index < $("#gridSelectSortTable tr").length - 1) {
                var clone = current.clone(true, true);
                current.remove();

                clone.insertAfter($("#gridSelectSortTable tr:eq(" + index + ")"));
            }
        })
    ...
    function colAdd() {
        $("#gridUnSelectCol :selected").appendTo("#gridSelectCol");
    }

    function colRemove() {
        $("#gridSelectCol :selected").appendTo("#gridUnSelectCol");
    }

    function colUp() {
        var first = $("#gridSelectCol :selected:first");
        var index = $("#gridSelectCol option").index(first);
        if (index > 0) {
            var selectedItems = $("#gridSelectCol :selected");
            $("#gridSelectCol :selected").remove();
            index = index - 1;
            selectedItems.insertBefore($("#gridSelectCol option:eq(" + index + ")"));
        }
    }

    function colDown() {
        var last = $("#gridSelectCol :selected:last");
        var index = $("#gridSelectCol option").index(last);
        if (index < $("#gridSelectCol option").length - 1) {
            var selectedItems = $("#gridSelectCol :selected");
            $("#gridSelectCol :selected").remove();
            index = index - (selectedItems.length - 1);
            selectedItems.insertAfter($("#gridSelectCol option:eq(" + index + ")"));
        }
    }
    return gridMvc;
})(window.jQuery);
    ...
    dateTimeFilterWidget.prototype.renderWidget = function () {
        var html = '<div class="form-group">\
                        ...
                        (this.datePickerIncluded ?
                            ...
                            '<div class="form-group">\
                                <label>' + this.lang.filterValueLabel + '</label>\
                                <input type="date" class="grid-filter-input form-control" value="' + this.value.filterValue + '" />\
                             </div>\
                             <div class="grid-filter-buttons">\
...
//sysCore custom
EnumFilterWidget = (function ($) {

    function enumFilterWidget() { }

    enumFilterWidget.prototype.getAssociatedTypes = function () { return ["System.Enum"]; };

    enumFilterWidget.prototype.showClearFilterButton = function () { return true; };

    enumFilterWidget.prototype.onRender = function (container, lang, typeName, values, cb, data) {
        this.data = data;
        this.cb = cb;
        this.container = container;
        this.lang = lang;
    ...
    enumFilterWidget.prototype.renderWidget = function () {
        var html = '<p>Filter:</p>\
                    <select class="grid-filter-type enumList form-control">\
                    </select>';
        this.container.append(html);
        var enumList = this.container.find(".enumList");
        enumList.append('<option value="">--Please Select--</option>');
        for (var i = 0; i < this.data.length; i++) {
            enumList.append('<option ' + (this.data[i].EnumKey.toString() == this.value.filterValue ? 'selected="selected"' : '') + ' value="' + this.data[i].EnumKey + '">' + this.data[i].DisplayText + '</option>');
        }
    };

    enumFilterWidget.prototype.registerEvents = function () {
        var enumList = this.container.find(".enumList");
        var $context = this;
...
CustomFilterWidget = (function ($) {
    function customFilterWidget(options) {
        this.options = options;
        this.options.findWidgetName = "widget_" + this.options.widgetName;
    }
    customFilterWidget.prototype.getAssociatedTypes = function () { return [this.options.widgetName]; };
    customFilterWidget.prototype.showClearFilterButton = function () { return true; };
    customFilterWidget.prototype.onRender = function (container, lang, typeName, values, cb) {
        this.cb = cb;
        this.container = container;
        this.lang = lang;
        this.value = values.length > 0 ? values[0] : { filterType: 1, filterValue: "" };//support only one filter value
    ...
    customFilterWidget.prototype.renderWidget = function () {
        var html = '<p>Filter:</p>\
                    <select class="grid-filter-type ' + this.options.findWidgetName + ' form-control">\
                    </select>';
        this.container.append(html);
        var customerList = this.container.find("." + this.options.findWidgetName);
        customerList.append('<option value="">--Please Select--</option>');
        var $this = this;
        $.post(this.options.url, function (responsedata) {
            $.each(responsedata, function (i, item) {
                customerList.append('<option ' + (item[$this.options.value] == $this.value.filterValue ? 'selected="selected"' : '') + '  value="' + item[$this.options.value] + '">' + item[$this.options.name] + '</option>');
            });
        });
    };
    customFilterWidget.prototype.registerEvents = function () {
        var customerslist = this.container.find("." + this.options.findWidgetName);
        var $context = this;
